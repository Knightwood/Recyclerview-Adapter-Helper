...
import com.kiylx.recyclerviewneko.ext.DefaultDecoration.Edge.Companion.computeEdge
...
class DefaultDecoration constructor(private val context: Context) : RecyclerView.ItemDecoration() {
    ...
    /**
     * 根据[orientation]值不同 [DividerOrientation.VERTICAL] 为是否显示顶部分割线
     * [DividerOrientation.HORIZONTAL] 为是否显示左侧分割线 [DividerOrientation.GRID]
     * 为最上/下侧是否显示分割线
     */
    var startVisible = false

    /**
     * 根据[orientation]值不同 [DividerOrientation.VERTICAL] 为是否显示底部分割线
     * [DividerOrientation.HORIZONTAL] 为是否显示右侧分割线 [DividerOrientation.GRID]
     * 为最左/右侧是否显示分割线
     */
    var endVisible = false

    /** 列表四周显示分割线 等于[startVisible]和[endVisible]全为true */
    var includeVisible
    ...
    /**
     * 展开分组条目后该条目是否显示分割线 控制[ItemExpand.itemExpand]为true的情况下是否显示分割线,
     * 但当你配置[onEnabled]后则无效, 因为此字段为其默认实现所用
     */
    var expandVisible = false

    /**
     * 分割线的方向, 仅支持[GridLayoutManager],
     * 其他LayoutManager都是根据其方向自动适应 布局管理器为[LinearLayoutManager] 和
     * [StaggeredGridLayoutManager] 时，该值会通过[adjustOrientation]自动调整
     */
    ...
    private var onEnabled: (BaseViewHolder.() -> Boolean)? = null

    /**
     * 根据回调函数来决定是否启用分割线
     *
     * @param block 函数返回值决定参数[BaseViewHolder]对应的Item是否启用分割线
     */
    fun onEnabled(block: BaseViewHolder.() -> Boolean) {
    ...
    //<editor-fold desc="颜色">
    /** 设置分割线颜色, 如果不设置分割线宽度[setDivider]则分割线宽度默认为1px 所谓分割线宽度指的是分割线的粗细, 而非水平宽度 */
    ...
    override fun getItemOffsets(
        ...
        onEnabled?.let {
            val vh = parent.findContainingViewHolder(view) as BaseViewHolder
            val modelOrNull = vh.getBindData<Any>()
    ...
    private fun drawHorizontal(canvas: Canvas, parent: RecyclerView, reverseLayout: Boolean) {
        ...
        loop@ for (i in 0 until parent.childCount) {
            ...
            if (onEnabled != null) {
                val vh = parent.getChildViewHolder(child) as BaseViewHolder
                val modelOrNull = vh.getBindData<Any>()
